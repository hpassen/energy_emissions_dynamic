...
# CANT IMPORT TO JS
# def filter_data(pp, variable, state):
#     '''
#     Returns a dataset for one state's energy 
#     '''
#     data = pd.read_json('./data.json')
#     gb = ['year']
#     tot_col, pp_col = VARS["gen"]
...
#         tot_col, pp_col = VARS["emit"]
#     # this should never happen - delete later
#     else:
#         return pd.DataFrame({0: [np.NaN]}).to_json(orient="records")
...
#     #group the data
#     if state == "USA":
#         output = data.groupby(gb)[col].sum()
    
#     else: 
#         state_data = data.loc[data.loc[:, 'state'] == state]
#         state_data = state_data.reset_index()
        
#         output = state_data.groupby(gb)[col].sum()
